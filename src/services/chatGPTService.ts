...
export const fetchQuizData = async ({ language }: ApiParams): Promise<QuizData> => {
    ...
    try {
        ...
        return {history: history, question: splitAnswers[0], correctAnswer: answers[0], wrongAnswers: answers.filter((value, index) => index !== 0)}
    } catch (error) {
